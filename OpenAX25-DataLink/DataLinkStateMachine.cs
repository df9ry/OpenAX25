...
namespace OpenAX25_DataLink
{
    ...
    internal sealed class DataLinkStateMachine
    {
        /// <summary>
        ...
        /// </summary>
        private readonly AX25Queue<byte[]> IFrameQueue = new AX25Queue<byte[]>();
        ...
        private readonly byte[][] IRxStore = new byte[128][];
        private readonly byte[][] ITxStore = new byte[128][];
            ...
            lock (this)
            {
                switch (p.PhysicalLayerPrimitiveType)
                {
                    ...
                    case PhysicalLayerPrimitive_T.PH_QUIET_Indication_T:
                        ...
                        break;
                    default:
                        m_runtime.Log(LogLevel.WARNING, m_config.name,
                            "Received unexpected primitive: " + p.PhysicalLayerPrimitiveTypeName);
                        break;
                } // end switch //
            }
        }
        ...
        private void Disconnected(AX25Payload f)
        {
            switch (f.FrameType)
            {
                case AX25Frame_T.UA:
        ...
        L1070:
            if (Layer3Initiated)
                goto L1071;
            IFrameQueue.Enqueue(((DL_DATA_Request)p).Data);
        ...
        private void AwaitingConnect(AX25Payload f)
        {
            switch (f.FrameType)
            {
                case AX25Frame_T.SABM:
        ...
        private void AwaitingConnect2_2(AX25Payload f)
        {
            switch (f.FrameType)
            {
                case AX25Frame_T.UI:
        ...
        L5181:
            if (V_S == V_A)
                goto L5171;
            IFrameQueue.Clear();
            OnDataLinkOutputEvent(new DL_CONNECT_Indication("???", m_version));
            goto L5171;
        ...
        private void AwaitingRelease(AX25Payload f)
        {
            switch (f.FrameType)
            {
                case AX25Frame_T.SABM:
        ...
        L3221:
            //??? OnLinkMultiplexerOutputEvent(new LM_SEIZE_Request());
            State = State_T.Connected;
            return;
        }

        private void Connected(AX25Payload f)
        {
            switch (f.FrameType)
            {
                case AX25Frame_T.SABM:
        ...
        L3231:
            ...
            SelectT1Value();
            P = true;
            if (IRxStore[N_R] != null)
                OnAX25OutputEvent(new AX25_I(IRxStore[N_R],
                    m_modulo, N_R, V_S, P, false, true));
            State = State_T.Connected;
            return;
        ...
        L3301:
            TraceLog("***** L3301");
            if (IRxStore[V_R] == null)
                goto L3311;
            OnDataLinkOutputEvent(new DL_DATA_Indication(IRxStore[V_R]));
            IRxStore[V_R] = null;
            V_R++;
            goto L3301;
        ...
        private void TimerRecovery(AX25Payload f)
        {
            switch (f.FrameType)
            {
                case AX25Frame_T.SABM:
        ...
        {
            OnAX25OutputEvent(new AX25_I(data, m_modulo, V_R, V_S, P, true, false));
            V_S++;
        }
        ...
        private void InvokeRetransmission()
        {
            ...
            do
            {
                ...
                if (I == null)
                    continue;
                OnAX25OutputEvent(new AX25_I(I, m_modulo, V_R, V_S,
                    false, false, true));
                V_S++;
            } while (V_S != x);
        }
        ...
        private void CheckNeedForResponse(AX25Payload f)
        {
            if (f.Command && P)
            {
                EnquiryResponse(true);
