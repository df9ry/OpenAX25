...
namespace OpenAX25Router
{
	...
	public class RouterChannel : L2Channel
    {
    	...
    	protected override void OnForward(L2Frame frame)
    	{
    		...
    		string text = String.Format("{0} [{1}]: {2}", header.ToString(), header.nextHop.ToString(),
    		                            L2HexConverter.ToHexString(data));
    		L2Runtime.Instance.Log(L2LogLevel.INFO, m_name, text);
    	}
